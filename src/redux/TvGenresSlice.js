...
export const fetchGenresTv = createAsyncThunk('genresTv/fetchGenresTv', async ({page,genres,year}) => {
    const urlGenresTv = url + `?page=${page}&with_genres=${genres}&year=${year}`
   
    try {
        const response = await axios.get(urlGenresTv, {
            headers: {
...
const genresTvSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchGenresTv.pending, (state) => {
            ...
            .addCase(fetchGenresTv.rejected, (state, action) => {
                state.status = 'failed'
