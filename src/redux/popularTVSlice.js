...
export const fetchTVPopular = createAsyncThunk('popularTV/fetchTVPopular', async (page) => {
    const urlUpComing = url + `/popular?language=vi-VN&page=${page}`
    try {
...
const popularTVSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTVPopular.rejected, (state, action) => {
                state.status = 'failed'
