...
const movieNowPlaySlice = createSlice({
    name: 'nowPlayMovies',
    initialState,
    reducers: {},
    extraReducers: (builder) => {
        builder
            .addCase(fetchMoviesNowPlay.pending, (state) => {
                state.status = 'loading'
            })
            .addCase(fetchMoviesNowPlay.fulfilled, (state, action) => {
                state.status = 'succeeded'
                state.items = action.payload
                state.total = action.payload.total_pages
                state.status = 'start'
            })
            .addCase(fetchMoviesNowPlay.rejected, (state, action) => {
                state.status = 'failed'
...
export default movieNowPlaySlice.reducer