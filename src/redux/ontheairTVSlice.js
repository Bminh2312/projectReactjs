...
export const fetchTVOnTheAir = createAsyncThunk('onTheAirTV/fetchTVOnTheAir', async (page) => {
    const urlUpComing = url + `/on_the_air?language=vi-VN&page=${page}`
    try {
...
const onTheAirTVSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchTVOnTheAir.pending, (state) => {
                state.status = 'loading'
            })
            .addCase(fetchTVOnTheAir.fulfilled, (state, action) => {
                state.status = 'succeeded'
                state.items = action.payload
                state.total = action.payload.total_pages
                state.status = 'start'
            })
            .addCase(fetchTVOnTheAir.rejected, (state, action) => {
                state.status = 'failed'
