...
import { fetchReviewTV } from '../../redux/reviewTVSlice';
...
export default function ReviewTV(props) {
  const { tv_id } = props;
  const [page, setPage] = useState(1)
  const { items, status, total } = useSelector(state => state.reviewTV);
  const dispatch = useDispatch();


  useEffect(() => {
    if (status === "start") {
      dispatch(fetchReviewTV({ id: tv_id, page: page }));
    }
  }, [page]);
