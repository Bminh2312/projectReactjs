...
import { fetchTVOnTheAir } from '../../redux/ontheairTVSlice';

export default function TVOnTheAir(props) {
    const {type} = props
    const [page, setPage] = useState(1);
    const [rowsToShow, setRowsToShow] = useState(2);
    const { items, total, status, path } = useSelector(state => state.onTheAirTV)
